...
con.query("CREATE TABLE IF NOT EXISTS users_rating ( \
    user_id INT, \
    data DATETIME, \
    appraiser INT, \
    mark INT CHECK (REGEXP_LIKE(mark, '^-?[0-9]+(\.[0-9]+)?$') AND mark BETWEEN 0 AND 6) NOT NULL, \
    about VARCHAR(900) CHECK (LENGTH(about) <= 900), \
    FOREIGN KEY (user_id) REFERENCES users(user_id) \
  )", (err, result)=> {
    if (err) throw err;
...
con.query("CREATE TABLE IF NOT EXISTS owner_rating ( \
    user_id INT, \
    data DATETIME, \
    appraiser INT, \
    mark INT CHECK (REGEXP_LIKE(mark, '^-?[0-9]+(\.[0-9]+)?$') AND mark BETWEEN 0 AND 6) NOT NULL, \
...
con.query("CREATE TABLE IF NOT EXISTS user_reports ( \
  user_id INT, \
  data DATETIME, \
  repotrer INT, \
  reason INT CHECK (REGEXP_LIKE(reason, '^[0-9]+(\.[0-9]+)?$') AND reason BETWEEN 0 AND 100) NOT NULL, \
  about VARCHAR(900) CHECK (LENGTH(about) <= 900), \
  FOREIGN KEY (user_id) REFERENCES users(user_id) \
  )", (err, result)=> {
  if (err) throw err;
...
con.query("CREATE TABLE IF NOT EXISTS flat_reports ( \
  flat_id INT, \
  data DATETIME, \
  repotrer INT, \
  reason INT CHECK (REGEXP_LIKE(reason, '^[0-9]+(\.[0-9]+)?$') AND reason BETWEEN 0 AND 100) NOT NULL, \
...
con.query("CREATE TABLE IF NOT EXISTS flat_status ( \
  flat_id INT, \
  data_unban DATETIME, \
  banned BOOLEAN DEFAULT FALSE, \
  realll BOOLEAN DEFAULT FALSE, \
...
// Автостворення акаунтів з owner
con.query("CREATE TABLE IF NOT EXISTS user_status ( \
  user_id INT, \
  data_unban DATETIME, \
  banned BOOLEAN DEFAULT FALSE, \
  realll BOOLEAN DEFAULT FALSE, \
...
con.query("CREATE TABLE IF NOT EXISTS comunal_img ( \
  flat_id INT, \
  user_id VARCHAR(256), \
  img VARCHAR(120) CHECK (LENGTH(img) <= 120), \
  comunal_name VARCHAR(70) CHECK (LENGTH(comunal_name) <= 70), \
